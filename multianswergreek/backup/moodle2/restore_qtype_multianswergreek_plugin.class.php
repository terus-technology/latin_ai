...
class restore_qtype_multianswergreek_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        // Add own qtype stuff.
        $elename = 'multianswergreek';
        $elepath = $this->get_pathfor('/multianswergreek');
    ...
    /**
     * Process the qtype/multianswergreek element
     */
    public function process_multianswergreek($data) {
        global $DB;
        ...
        // question_multianswergreek too.
        if ($questioncreated) {
            // Adjust some columns.
            $data->question = $newquestionid;
            // Note: multianswergreek->sequence is a list of question->id values. We aren't
            ...
            // Insert record.
            $newitemid = $DB->insert_record('question_multianswergreek', $data);
            // Create mapping (need it for after_execute recode of sequence).
            $this->set_mapping('question_multianswergreek', $oldid, $newitemid);
        }
    }
     ...
     *
     * For multianswergreek qtype we use it to restore the sequence column,
    ...
    public function after_execute_question() {
        ...
        foreach ($rs as $rec) {
            ...
            if (substr_count($rec->sequence, ',') + 1 != count($sequencearr)) {
                $this->task->log('Invalid sequence found in restored multianswergreek question ' . $rec->id, backup::LOG_WARNING);
            }
     ...
     * recoded pointing to all the restored stuff for multianswergreek questions
     *
     * answer is one comma separated list of hypen separated pairs
     * containing sequence (pointing to questions sequence in question_multianswergreek)
