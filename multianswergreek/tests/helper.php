...
/**
 * Test helpers for the multianswergreek question type.
 *
...
global $CFG;
require_once($CFG->dirroot . '/question/type/multianswergreek/question.php');

/**
 * Test helper class for the multianswergreek question type.
 *
 * @copyright  2013 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswergreek_test_helper extends question_test_helper {
    /**
     * Get test questions
     *
     * @return array
    ...
    public function get_test_questions() {
        return ['twosubq', 'fourmc', 'numericalzero', 'dollarsigns', 'multiple'];
    }

    /**
     * Makes a multianswergreek question about completing two blanks in some text.
     *
     * @return qtype_multianswergreek_question
     */
    public function make_multianswergreek_question_twosubq() {
        question_bank::load_question_definition_classes('multianswergreek');
        $q = new qtype_multianswergreek_question();
        test_question_maker::initialise_a_question($q);
        $q->name = 'Simple multianswergreek';
        ...
        $q->qtype = question_bank::get_qtype('multianswergreek');
        ...
        $sa->name = 'Simple multianswergreek';
        ...
        $mc->name = 'Simple multianswergreek';
     ...
     * Makes a multianswergreek question about completing two blanks in some text.
     *
     * @return object the question definition data, as it might be returned from get_question_options.
     */
    public function get_multianswergreek_question_data_twosubq() {
        $qdata = new stdClass();
        test_question_maker::initialise_question_data($qdata);

        $qdata->name = 'Simple multianswergreek';
        $qdata->questiontext =
        ...
        $mc->options->answers = [
            23 => new question_answer(23, 'Bow-wow', 0, 'You seem to have a dog obsessions!', FORMAT_HTML),
            24 => new question_answer(24, 'Wiggly worm', 0, 'Now you are just being ridiculous!', FORMAT_HTML),
            25 => new question_answer(25, 'Pussy-cat', 1, 'Well done!', FORMAT_HTML),
        ];
    ...
    /**
     * Makes a multianswergreek question onetaining one blank in some text. This question has no hints.
     *
     * @return object the question definition data, as it might be returned from get_question_options.
     */
    public function get_multianswergreek_question_data_dollarsigns() {
        $qdata = new stdClass();
        test_question_maker::initialise_question_data($qdata);

        $qdata->name = 'multianswergreek with $s';
        $qdata->questiontext =
        ...
        $mc->name = 'multianswergreek with $s';
     ...
     * Makes a multianswergreek question about completing two blanks in some text.
     *
     * @return object the question definition data, as it might be returned from the question editing form.
     */
    public function get_multianswergreek_question_form_data_twosubq() {
        $formdata = new stdClass();
        $formdata->name = 'Simple multianswergreek';
        $formdata->questiontext = [
            'text' => 'Complete this opening line of verse: "The ' .
        ...
        $formdata->generalfeedback = [
            'text' => 'General feedback: It\'s from "The Owl and the Pussy-cat" ' .
     ...
     * @return qtype_multianswergreek_question
     */
    public function make_multianswergreek_question_fourmc() {
        question_bank::load_question_definition_classes('multianswergreek');
        $q = new qtype_multianswergreek_question();
        test_question_maker::initialise_a_question($q);
        $q->name = 'multianswergreek four multi-choice';
        $q->questiontext = '
            <p>Match the following cities with the correct state:</p>
            <ul>
        ...
        foreach ($subqdata as $i => $data) {
            ...
            $mc = new qtype_multichoice_single_question();
            test_question_maker::initialise_a_question($mc);
            $mc->name = 'multianswergreek four multi-choice';
            $mc->questiontext = $data['qt'];
            $mc->questiontextformat = FORMAT_HTML;
            $mc->generalfeedback = '';
            $mc->generalfeedbackformat = FORMAT_HTML;

            $mc->shuffleanswers = 0; // This is a cheat to make the unit tests easier to write.
            // In reality, multianswergreek questions always shuffle.
            $mc->answernumbering = 'none';
            $mc->layout = qtype_multichoice_base::LAYOUT_DROPDOWN;

            $mc->answers = [
                10 * $i => new question_answer(
                    13,
                    'California',
                    (float) ($data['California'] == 'OK'),
                    $data['California'],
                    FORMAT_HTML
                ),
                10 * $i + 1 => new question_answer(
                    14,
                    'Arizona',
                    (float) ($data['Arizona'] == 'OK'),
                    $data['Arizona'],
                    FORMAT_HTML
                ),
            ];
    ...
    /**
     * Makes a multianswergreek question with one numerical subquestion, right answer 0.
     * This is used for testing the MDL-35370 bug.
     *
     * @return qtype_multianswergreek_question
     */
    public function make_multianswergreek_question_numericalzero() {
        question_bank::load_question_definition_classes('multianswergreek');
        $q = new qtype_multianswergreek_question();
        test_question_maker::initialise_a_question($q);
    ...
    /**
     * Makes a multianswergreek question with multichoice_multiple questions in it.
     *
     * @return qtype_multianswergreek_question
     */
    public function make_multianswergreek_question_multiple() {
        question_bank::load_question_definition_classes('multianswergreek');
        $q = new qtype_multianswergreek_question();
        test_question_maker::initialise_a_question($q);
        ...
        $mc->answers = [
            16 => new question_answer(16, 'Apple', 0.3333333, 'Good', FORMAT_HTML),
            17 => new question_answer(17, 'Burger', -0.5, '', FORMAT_HTML),
            18 => new question_answer(18, 'Hot dog', -0.5, 'Not a fruit', FORMAT_HTML),
            19 => new question_answer(19, 'Pizza', -0.5, '', FORMAT_HTML),
            20 => new question_answer(20, 'Orange', 0.3333333, 'Correct', FORMAT_HTML),
            21 => new question_answer(21, 'Banana', 0.3333333, '', FORMAT_HTML),
        ];
        $mc->qtype = question_bank::get_qtype('multichoice');
        $mc->maxmark = 1;
        ...
        $mc2->answers = [
            22 => new question_answer(22, 'Raddish', 0.5, 'Good', FORMAT_HTML),
            23 => new question_answer(23, 'Chocolate', -0.5, '', FORMAT_HTML),
            24 => new question_answer(24, 'Biscuit', -0.5, 'Not a vegetable', FORMAT_HTML),
            25 => new question_answer(25, 'Cheese', -0.5, '', FORMAT_HTML),
            26 => new question_answer(26, 'Carrot', 0.5, 'Correct', FORMAT_HTML),
        ];
